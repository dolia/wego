...
func (g *Graph) GetService(p reflect.Type) reflect.Value  {

	var val reflect.Value

	for _,item := range g.Objects(){
		if p == item.reflectType || p.String() == item.Name {
			val = item.reflectValue
			return val
		}
	}
...
func parseTag(t string) (*tag, error) {
	found, value, err := structtag.Extract("service", t)
	if err != nil {
		return nil, err
	}
	if !found {

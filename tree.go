...
func (n *node) addRoute(path string, handle Handle,f string,middleWares []MiddleWare) {
	...
	walk:
		for {
			...
			if i < len(n.path) {
				...
				n.handle = nil
				n.f = ""
				n.middleWares = make([]MiddleWare,0)
				n.wildChild = false
			}
			...
			if i < len(path) {
				...
				n.insertChild(numParams, path, fullPath, handle,f,middleWares)
				return
...
func (n *node) insertChild(numParams uint8, path, fullPath string, handle Handle,f string,middleWares []MiddleWare) {
...
// given path.
func (n *node) getValue(path string) (handle Handle, p Params, f string,middleWares []MiddleWare ,tsr bool) {
